...
module.exports = {
  run: async (toolbox: GluegunToolbox) => {
    ...
    const { wallet, signer, tokenId, registry } = await toolbox.config.loadConfig(true)
    if (tokenId !== 0) {
      toolbox.print.error(`DID NFT already minted : ${tokenId}`)
    } else {
      const tokenId = await Caelum.mintNft(wallet, registry, signer['CaelumOrg'].keypair.public_key)
